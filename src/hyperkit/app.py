...
class Game:
    ...
    def __init__(
        ...
        height: int = 1280,
        fps: int = 60,
        background_color: tuple[float, float, float, float] = (0.08, 0.08, 0.1, 1),
    ) -> None:
        self.title = title
        self.width = width
        self.height = height
    ...
    def run(self) -> None:
        if self.scene is None:
            raise RuntimeError("No scene set. Use Game(...).set_scene(MyScene()).run().")
            ...
            def _redraw(self):
                ...
                with self.canvas:
                    ...
                    for obj in game.scene.objects:
                        ...
                        Color(*obj.color)
                        if obj.shape == "circle":
                            Ellipse(pos=(obj.x, obj.y), size=(obj.width, obj.height))
                        else:
                            Rectangle(pos=(obj.x, obj.y), size=(obj.width, obj.height))
            ...
            def on_touch_up(self, touch):
                ...
                if game.scene:
                    ...
                    if gesture and gesture.kind == "tap":
                        ...
                        game.scene.on_swipe(gesture.start, gesture.end, gesture.direction)
                return True
