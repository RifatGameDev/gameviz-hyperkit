...
TEMPLATE_MAIN = Path("src/hyperkit/templates/simple_physics/main.py")
TEMPLATE_README = Path("src/hyperkit/templates/simple_physics/README.md")
PHASE_DOC = Path("docs/SIMPLE_PHYSICS_POLISH_PHASE53.md")
...
def test_simple_physics_main_exists_and_has_valid_syntax():
...
def test_simple_physics_main_uses_core_hyperkit_features():
    content = read(TEMPLATE_MAIN)
...
def test_simple_physics_main_has_clear_scene_and_entry_point():
    content = read(TEMPLATE_MAIN)

    assert "class SimplePhysicsScene(Scene):" in content
...
def test_simple_physics_main_has_physics_gameplay_logic():
    content = read(TEMPLATE_MAIN)
...
def test_simple_physics_template_readme_is_polished():
    assert TEMPLATE_README.exists()

    content = read(TEMPLATE_README)

    assert "# Simple Physics Template" in content
...
def test_simple_physics_phase_doc_exists():
    assert PHASE_DOC.exists()

    content = read(PHASE_DOC)

    assert "Simple Physics Polish - Phase 53" in content
    assert "first production-quality polish pass" in content
    assert "hyperkit validate-templates" in content


def test_main_readme_links_simple_physics_polish_doc():
    content = read(README)

    assert "Simple Physics Polish - Phase 53" in content
    assert "docs/SIMPLE_PHYSICS_POLISH_PHASE53.md" in content


def test_health_checks_include_simple_physics_polish_doc():
    from hyperkit import generate_health_report

    report = generate_health_report(".")
    paths = {check.path for check in report.checks}

    assert "docs/SIMPLE_PHYSICS_POLISH_PHASE53.md" in paths
    assert "tests/test_simple_physics_polish_phase53.py" in paths


def test_release_checks_include_simple_physics_polish_doc():
    from hyperkit import generate_release_report

    report = generate_release_report(".")
    check_names = {check.name for check in report.checks}

    assert "Required release file: docs/SIMPLE_PHYSICS_POLISH_PHASE53.md" in check_names
    assert (
        "Required release test: tests/test_simple_physics_polish_phase53.py"
        in check_names
    )


def test_simple_physics_files_have_no_local_paths_or_escaped_newlines():
