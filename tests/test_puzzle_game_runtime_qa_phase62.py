...
EVIDENCE_ROOT = Path(
    "docs/release-evidence/templates/puzzle_game"
)
...
def test_puzzle_game_runtime_evidence_files_exist():
    required_files = [
        MANUAL_RESULT,
        VALIDATION_OUTPUT,
        RUNTIME_NOTES,
...
def test_puzzle_game_screenshot_is_valid_png():
...
def test_puzzle_game_tracker_entry_is_complete():
    tracker = json.loads(read(TRACKER_PATH))
    entry = tracker["templates"]["puzzle_game"]

    assert entry["status"] in {"pass", "pass_with_notes"}
    assert (
        entry["result_file"]
        == "templates/puzzle_game/manual-qa-result.md"
    )


def test_puzzle_game_manual_result_is_completed():
    content = read(MANUAL_RESULT)

    assert "Template name: Puzzle Game" in content
    assert (
...
def test_puzzle_game_runtime_notes_are_completed():
    content = read(RUNTIME_NOTES)

    assert "# Puzzle Game Runtime Notes" in content
    assert "Correct tile increased the score: Yes" in content
    assert "Wrong tile ended the run: Yes" in content
    assert "Tap after failure restarted the puzzle: Yes" in content
    assert "Tap after completion restarted the puzzle: Yes" in content
    assert "All nine puzzle steps were reachable: Yes" in content
    assert (
        "Final result: PASS" in content
        or "Final result: PASS WITH NOTES" in content
    )
...
def test_puzzle_game_validation_output_is_not_empty():
...
def test_puzzle_game_evidence_has_no_local_paths():
    files = [
        MANUAL_RESULT,
        VALIDATION_OUTPUT,
        RUNTIME_NOTES,
...
def test_release_evidence_report_marks_puzzle_game_complete():
    ...
    assert "puzzle_game" in report.complete_templates
